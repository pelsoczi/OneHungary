...
class DetailsDataSourceTest {
    ...
    fun `request offers, 200 success`() = runTest {
        // given
        enqueueMockResponse(200, "details-200-success.json")
        // when
        detailsDataSource.request().test {
            // then
            awaitItem().let {
    ...
    fun `request invalid url`() = runTest {
        // given
        enqueueMockResponse(400, "details-400-invalid-url.json")
        // when
        detailsDataSource.request().test {
            // then
            awaitError()
    ...
    private fun enqueueMockResponse(responseCode: Int, fileName: String) {
        val response = MockResponse()
            ...
            .setBody(readJsonFile("com/onehungary/one/api/details/$fileName"))
        mockWebServer.enqueue(response)
    }
